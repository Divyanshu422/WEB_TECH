  ...
  return (
    <nav className={`sticky top-4 z-40 flex items-center justify-between rounded-xl p-2 ml-4 backdrop-blur-xl ${
      ...
      <div className="ml-[6px]">
        <p className={`text-[33px] capitalize ${darkMode ? 'text-white' : 'text-stone-950'}`}>
          <Link to="#" className="font-bold capitalize hover:text-black hover:scale-105 transition duration-300 ease-in-out">
            <div className={`text-sm ${darkMode ? 'text-[#D1D1D2]' : 'text-stone-950'}`}>
              Hello Admin
            </div>
            <motion.div className='text-3xl'
              ...
              transition={{ staggerChildren: 0.1, duration:3 }}>
                {displayData1.split('').map((char, index) => {
                        return (
                            <motion.span key={index} variants={defaultAnimations} className={darkMode ? 'text-[#D1D1D2]' : 'text-stone-950'}>
                            {char}
                            </motion.span>
                        );
                        })}
      ...
      <div className='flex gap-x-2'>
        <div className={`relative flex h-[50px] w-[260px] items-center rounded-xl px-2 py-1  shadow-2xl ${
          darkMode
          ...
          <div className="flex items-center px-4"> 
            ...
            <input
              ...
              className={`absolute right-7 outline-none p-1 ${darkMode ? 'bg-[#1D1F25]  text-[#D1D1D2' : 'bg-white dark:bg-[#0b14374d]'}`}
            />
          </div>
        </div>
            ...
            <img
              ...
              className={`absolute top-2 h-8 w-8 rounded-full ${
                darkMode
                  ...
                  : 'bg-slate-300 dark:bg-[#0b14374d] dark:text-white'
              }`}
              alt="profile"
            />
          </div>  
